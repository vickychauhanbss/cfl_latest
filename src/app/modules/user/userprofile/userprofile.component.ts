...
export class UserprofileComponent implements OnInit {
	...
	OnloadPopup(video) {
		this.dialog.open(WelcomeComponent, {
			data: { 'check': 'userprofile', video: video.video_path }
		});
	}
		...
		if (this.userData != null) {
			...
			this.userService.get_user_profile_data({ 'user_id': this.userId, 'loggedin_user': this.check_Id }).subscribe((data: any) => {
				...
				if (data.status == 'OK') {
					...
					this.toastr.errorToastr('User does not exist any more.', 'Oops!', {
						position: 'top-full-width', toastTimeout: 3000, animate: 'slideFromTop', showCloseButton: true
					});
					(this.inChild)?this.back.emit('closed'):this.backToDashboard();
				}
			}, error => {
				this.spinner.hide();
			})
